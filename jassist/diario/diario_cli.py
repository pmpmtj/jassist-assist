...
# Set up logger
logger = setup_logger("diario_cli", module="diario")

def parse_diario_text(input_text: str, transcription_id: Optional[int] = None, 
                     test_mode: bool = False) -> Tuple[bool, Dict[str, Any]]:
    """
    Parse diary text and return structured data.
    
    This function can be imported and used programmatically.
    
    Args:
        input_text: The text to parse for diary information
    ...
    """
    try:
        logger.debug(f"Processing diary text: {input_text[:50]}...")
        ...
        if test_mode:
            ...
            mock_response = {
                "conteudo": input_text,
                "estado_espirito": "pensativo",
                "etiquetas": ["reflexão_pessoal"]
            }
            
            logger.debug(f"Generated mock diary data: {mock_response}")
            return True, mock_response
        else:
            # Import here to avoid circular imports
            from jassist.diario.diario_processor import process_diary_entry
            
            try:
                # Process the entry
                success, diary_data = process_diary_entry(
                    text=input_text, 
                    db_id=transcription_id
                )
                
                if not success:
                    logger.error("Failed to process diary entry")
                    if isinstance(diary_data, dict) and "error" in diary_data:
                        return False, diary_data
                    return False, {"error": "Failed to process diary entry"}
                    
                return True, diary_data
            except Exception as e:
                import traceback
                error_traceback = traceback.format_exc()
                logger.error(f"Exception in process_diary_entry: {e}\n{error_traceback}")
                return False, {"error": str(e), "traceback": error_traceback}
        
    except Exception as e:
        import traceback
        error_traceback = traceback.format_exc()
        logger.exception(f"Error parsing diary text: {e}")
        return False, {"error": str(e), "traceback": error_traceback}
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="Process diary entries from text input")
    ...
    # Process the text
    success, result = parse_diario_text(
